...
router.post("/", async (req, res) => {
  try {
    ...
    const orderData = {
      ...
      rawSubtotalUsd: Number(rawSubtotalUsd) || verifiedTotalUsd,
      rawSubtotalUzs: Number(rawSubtotalUzs) || Math.round((Number(rawSubtotalUsd) || verifiedTotalUsd) * computedUsdRate),
      discountAmount: Number(discountAmount) || (computedDiscountUsd * computedUsdRate),
      discountUsd: Number(discountUsd) || computedDiscountUsd,
      discountUzs: Number(discountUzs) || Math.round((Number(discountUsd) || computedDiscountUsd) * computedUsdRate),
      promoCode: promoCode || "",
      extraPhone: sanitizeText(extraPhone || (customerProfile && customerProfile.extraPhone) || ""),
      telegram: sanitizeText(telegram || (customerProfile && customerProfile.telegram) || ""),
      customerProfile: (customerProfile && typeof customerProfile === "object") ? customerProfile : {},
