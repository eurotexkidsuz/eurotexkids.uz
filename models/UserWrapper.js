...
class UserMock {
    constructor(data) {
        this._id = (data && data._id) ? data._id.toString() : Math.random().toString(36).substring(2, 11);
        this.email = "";
        this.code = null;
    ...
    async save() {
        const db = readDB();
        const idx = db.findIndex(u => u.email && u.email.toLowerCase() === this.email.toLowerCase());
        this.updatedAt = new Date();
...
User.findOne = async function(query) {
    if (isDbConnected()) {
        try {
            ...
            console.error("Mongoose findOne failed, falling back to local DB:", err.message);
        }
    }
    ...
    const userObj = db.find(u => {
        for (let key in query) {
            const val = query[key];
            if (key === "email") {
                if (!u.email || u.email.toLowerCase() !== val.toLowerCase()) return false;
...
User.findById = async function(id) {
    if (isDbConnected()) {
        try {
            ...
            console.error("Mongoose findById failed, falling back to local DB:", err.message);
        }
    }
...
User.find = async function(query) {
    if (isDbConnected()) {
        try {
            ...
            console.error("Mongoose find failed, falling back to local DB:", err.message);
        }
    }
