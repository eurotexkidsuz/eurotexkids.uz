(function () {
    ...
    async function generateQrCode() {
        ...
        try {
            ...
            if (qrStatusText) qrStatusText.textContent = 'Server bilan bog\'lanishda xatolik yuz berdi';
            return;
        }

        // Build scan URL
        var loginUrl = window.location.origin + '/users/qr-auth?token=' + qrSessionToken;

        if (typeof QRCode === 'undefined') {
            qrSpinner.style.display = 'none';
            if (qrStatusText) qrStatusText.textContent = 'QR kutubxonasi yuklanmadi. Sahifani yangilang.';
            return;
        }

        var tempCanvas = document.createElement('canvas');
        QRCode.toCanvas(tempCanvas, loginUrl, {
            width: 200, margin: 2,
            color: { dark: '#1a0050', light: '#ffffff' }
        }, function (err) {
            if (err) {
                if (qrStatusText) qrStatusText.textContent = 'QR kod yaratishda xatolik yuz berdi';
                qrSpinner.style.display = 'none';
                return;
            }
            ...
            qrSpinner.style.display = 'none';
            if (qrStatusText) qrStatusText.textContent = 'Telefon kamerangiz bilan skanerlang';
    ...
    function showExpired() {
        ...
        if (qrTimerRow)       qrTimerRow.style.display = 'none';
        if (qrStatusText)     qrStatusText.textContent = '⏰ QR kod muddati tugadi. Yangi kod oling.';
        ...
        qrPollInterval = setInterval(async function () {
            ...
            try {
                ...
                if (d.status === 'authorized' && d.rememberToken) {
                    ...
                    window.location.href = '/profile.html?email=' + encodeURIComponent(d.email || '');
